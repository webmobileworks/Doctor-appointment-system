        ...
        <div className="hidden md:flex items-center gap-2">
          ...
          {user ? (
            <div className="flex items-center gap-3">
              <Link 
                to={user.role === 'admin' ? "/admin" : user.role === 'doctor' ? "/doctor-dashboard" : "/profile"} 
              ...
              >
                ...
                <span className="text-sm font-medium text-primary">
                  {user.role === 'admin' ? "Admin" : user.role === 'doctor' ? "Portal" : user.name}
                </span>
              </Link>
              ...
              {user ? (
                <div className="flex flex-col gap-2 pt-2 border-t border-border/50">
                  <Link 
                    to={user.role === 'admin' ? "/admin" : user.role === 'doctor' ? "/doctor-dashboard" : "/profile"} 
                    onClick={() => setMobileOpen(false)} 
                    className="flex items-center gap-2 px-4 py-3 bg-muted/50 rounded-xl hover:bg-muted transition-colors"
                  >
                    <User className="w-4 h-4 text-primary" />
                    <span className="text-sm font-medium">
                      {user.role === 'admin' ? "Admin Dashboard" : user.role === 'doctor' ? "Doctor Portal" : user.name}
                    </span>
                  </Link>
